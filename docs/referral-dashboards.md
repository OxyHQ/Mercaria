...
add one — its own contract, not a preference: on web the DOCUMENT scrolls
(Bloom's `AppShell scroll="document"`) and an inner scroller would break the
shell's sticky navigation and its `ContentPanel`'s sticky overlays, and on
native the shell already puts the body in a full-height `ScrollView`, so a
second one is two nested scrollers. This screen shipped with one, and it was measured out
...
`ScreenShell` lives in `packages/ui/src/components/shell/` (the storefront's
`components/shell/ScreenShell.tsx` re-exports it, and the dashboard's and POS's
`Screen` wrap it), so this is a hazard for every screen that mounts it, not one
narrow to this dashboard — check for an inner `ScrollView`
