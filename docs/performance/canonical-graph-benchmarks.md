...
- **`offers_merchant_browse_idx` and `offers_storefront_browse_idx` have no
  reader in `services/`.** #57 built them for the issue's index 3 and the
  merchant storefront page that would use them belongs to **#84**. X01 measures
  the shape at 0.251 ms so #84 inherits a number; the indexes stay.
...
- **#74** — ranking. X02/X03 give it the dedup strategy comparison; Q15 gives it
  ...
  projection.
- **#84** — the merchant storefront page, whose indexes already exist and are
  measured (X01).
