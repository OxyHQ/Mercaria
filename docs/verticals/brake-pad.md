...
| Prove vehicle selector and reverse fitment display | Done at the SERVICE level. **There is no HTTP surface** — the compatibility domain ships its reads and its gates and `docs/compatibility.md` assigns the routes to the storefront work that renders them |
