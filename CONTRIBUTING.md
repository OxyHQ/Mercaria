...
Coding standards for this repository are in `docs/`, indexed by [`docs/README.md`](docs/README.md), and this is a repository where reading the relevant domain document first genuinely pays: the multi-currency model (`docs/currency.md` — native catalog prices, `DualMoney` on anything transacted, and no currency as a settlement invariant) is easy to get subtly wrong, and "report" means two unrelated things in this codebase, store sales analytics and abuse reports (`docs/moderation.md`), which must never be merged. `AGENTS.md` at the repository root is the product description and the pointer into `docs/`; it is read directly by Claude Code, Codex, Cursor and Copilot, and the domain document is the file to update when a convention changes.
