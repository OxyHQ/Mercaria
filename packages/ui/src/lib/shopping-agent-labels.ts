...
/**
 * `ShoppingAgentCard`'s own copy (#437).
 *
 * ## Every frame here carries its own slots, including its separators
 *
 * The four `lastLooked*` keys are the shape to read. What they replace was one
 * sentence plus a `\` · next ${…}\`` glued onto it at the render site, and the
 * separator was IN the fragment — so a language that joins two clauses
 * differently had no way to say so, and a right-to-left one got a mid-sentence
 * `·` whose side is decided by the surrounding run. Four complete frames chosen
 * by branch cost three extra keys and take that decision back into the words.
 * Same reason `notifyPolicyQuiet` is a whole second sentence rather than a
 * suffix.
 *
 * ## No slot here is named `count`, deliberately
 *
 * `%{count}` is what i18n-js pluralises on, and #436 pins this package's plural
 * residual exactly in both directions. `watching` therefore reads
 * `Watching (%{things})` at every quantity rather than branching on one — the
 * pre-#437 code branched to avoid `Watching 1 things`, which is a hand-rolled
 * English plural that has no correct form in `ru` or `ar` anyway. A parenthesised
 * count is right at every number in all twelve. `merchantsExcluded` is restated
 * as a labelled value for the same reason: `N merchant(s) excluded` is a counted
 * noun wearing a workaround.
 *
