...
/**
 * A coarse duration in the largest unit that still describes it — `30 seconds`,
 * `5 minutes`, `6 hours`, `2 days` — spelled for `locale` (#437).
 *
 * ## Why `Intl` rather than four translation keys
 *
 * This was four English template literals inside `ShoppingAgentCard`, and the
 * obvious extraction is four keys with a number slot. That is the wrong shape,
 * for a reason the slot name makes visible: a counted noun does not have one
 * form per language. `%{count}` would drag in i18n-js pluralisation and move
 * #436's `pluralCategoryResidual` — six Arabic categories and four Russian ones
 * per key, twelve locales deep — and any OTHER slot name buys plural-safety by
 * writing `1 minutes`, which is the bug the plural forms exist to prevent.
 *
 * `Intl.NumberFormat`'s `style: "unit"` already knows every one of those forms.
 * It renders `1 minute`/`30 minutes` in `en`, `1 минута`/`5 минут`/`30 минут`
 * in `ru`, and the Arabic dual in `ar`, from CLDR data rather than from copy
 * anybody has to maintain. So this removes four English strings and adds NO
 * translation keys and NO plural forms at all.
 *
 * The fallback is the pre-#437 English, per this module's rule: a runtime whose
