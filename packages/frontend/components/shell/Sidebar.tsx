...
/**
 * Storefront sidebar — a thin app wrapper over the shared {@link AppSidebar}. It
 * feeds Mercaria's Shop-style {@link NAV_ITEMS} (ungated, but destinations whose
 * screen isn't built yet render as dimmed, non-interactive rows via `disabled`),
 * builds the header (logo → home + collapse trigger) and footer (the
 * ProfileButton account switcher), and delegates the rail chrome, collapse
...
export function Sidebar() {
  ...
  // Every destination is rendered; the not-yet-built ones (`available: false`)
  // become dimmed, non-interactive rows so pressing them is a safe no-op (the
  // shared `SidebarRow` disables the Pressable when `disabled`).
