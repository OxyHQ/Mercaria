...
await esbuild.build({
  ...
  //
  // @oxy.so/* MUST stay external. The @oxy.so/crowdsource* packages are published as
  ...
  // @oxy.so/crowdsource-contracts/dist/primitives.js, reached through
  // @oxy.so/crowdsource-express — the throwing frame is not the package you imported.
  ...
  //   @oxy.so/core 13.0.0, dist/esm — 223 relative imports, 212 ending in .js and
  //     the other 11 being ./locales/*.json carrying `with { type: "json" }`.
  //     ZERO extensionless, so Node resolves every one.
  //   @oxy.so/bloom 0.67.0, lib/module — 82 relative imports, ALL 82 extensionless.
