...
/** Tracked by `epoch`: the row's own id is generated by the table. */
const saltEpochs: number[] = [];
...
/**
 * A RETIRED pseudonym-salt epoch with a caller-supplied deadline.
 *
 * `active_until` is always set, for two reasons that both matter here: the
 * partial unique `analytics_pseudonym_salts_current_key` admits exactly one row
 * with a NULL `active_until` in the whole database, so two fixtures — or one
 * fixture and a sibling file — would collide on it; and a RETIRED epoch is the
 * only kind the sweep is ever meant to reach.
 *
 * `epoch` is unique table-wide and is minted from a large random offset rather
 * than a counter, because the database is shared and a sibling that opens a real
 * epoch starts at 1.
 */
async function makeSaltRow(expiresAt: Date): Promise<number> {
  const epoch = 900_000_000 + Math.floor(Math.random() * 90_000_000);
  saltEpochs.push(epoch);
  const activeFrom = new Date(Date.now() - 2 * 60 * 60 * 1_000);
  await db.insert(analyticsPseudonymSalts).values({
    epoch,
    salt: `sweep-salt-${uuidv7()}`,
    activeFrom,
    activeUntil: new Date(activeFrom.getTime() + 60 * 1_000),
    expiresAt,
  });
  return epoch;
}

async function saltExists(epoch: number): Promise<boolean> {
  const rows = await db
    .select({ epoch: analyticsPseudonymSalts.epoch })
    .from(analyticsPseudonymSalts)
    .where(eq(analyticsPseudonymSalts.epoch, epoch));
  return rows.length === 1;
}
...
describe('sweepExpiredRowsOnce — the tick the scheduler runs', () => {
  ...
  it('reaps a retired salt past its stamped deadline and leaves one still inside it', async () => {
    // #77 data-lifecycle rule 7, and the ONE case in this file whose absence was
    // load-bearing rather than merely untested. `identity.ts` stamps
    // `expires_at = opened_at + 45d`; the registry entry then named the SAME
    // 45 days as its `retentionSeconds`, and the sweep deletes where
    // `column <= now() - retentionSeconds` — so the window was applied twice and
    // the salt survived to 90 days, exactly as long as the discovery events
    // derived under it. Nothing else could see that: the registry census two
    // describes up asserts the table is PRESENT, every functional analytics test
    // passes either way, and `countOverdueSalts` reads the stamped deadline, so
    // its "must be zero" health counter reported every epoch overdue for 45 days
    // and would have been muted as noise long before anybody traced it.
    //
    // An hour past the deadline is enough BECAUSE the entry is now
    // `retentionSeconds: 0`. That is what makes this case a real discriminator
    // rather than a slow one: restore the doubled window and the expired fixture
    // survives the tick, and this is the only assertion in the suite that moves.
    const expired = await makeSaltRow(EXPIRED_AT);
    const live = await makeSaltRow(NOT_YET_EXPIRED_AT);

    await sweepExpiredRowsOnce();

    expect(await saltExists(expired), 'the retired salt survived its deadline').toBe(false);
    expect(await saltExists(live), 'a salt inside its retention was reaped').toBe(true);
  });
  ...
  it('covers every target in ONE tick, not just the first', async () => {
    ...
    const salt = await makeSaltRow(EXPIRED_AT);
    ...
    expect(await saltExists(salt)).toBe(false);
  });
});
