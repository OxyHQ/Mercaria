...
describe('the registry the sweeper runs over', () => {
  it('names every table the sweeper is responsible for, and nothing else', () => {
    ...
    // they need this loop. `guest_sessions` appears TWICE deliberately: a
    // session is purged 7 days after ABSOLUTE expiry or after REVOCATION
    // (ADR 0003 D11), and the registry has no way to express OR — so each
    // trigger is its own target over its own indexed column.
