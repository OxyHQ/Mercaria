...
async function processSyncJob(job: Job<MarketplaceSyncJobData>): Promise<void> {
  switch (job.name) {
    ...
    case JOB_CONNECTION_RECONCILE:
      ...
      return;
    case JOB_CONNECTION_WEBHOOK_REREGISTER:
      await handleConnectionWebhookReregister(job.data as ConnectionWebhookReregisterJob);
      return;
    ...
    case JOB_WEBHOOK_PROCESS:
      ...
      return;
    case JOB_PRODUCT_PUSH:
      await handleProductPush(job.data as ProductPushJob);
      return;
    case JOB_ORDER_SYNC:
