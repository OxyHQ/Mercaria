...
import { FEED_SECTIONS } from '../lib/mock-products.js';

/**
 * Home feed API.
 *
 * PUBLIC — browsing products is available to anonymous viewers, so this route
 * applies NO auth middleware. It currently serves mock sections (see
...
/**
 * GET /feed
 * The home feed: an ordered list of discriminated sections (products + merchants).
 */
router.get('/', (_req, res) => {
  const body: ApiResponse<Feed> = {
    success: true,
    data: { sections: FEED_SECTIONS },
  };
