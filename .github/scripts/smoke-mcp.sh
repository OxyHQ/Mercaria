...
MCP_ORIGIN="${MCP_ORIGIN:-https://mcp.mercaria.oxy.so}"
...
if ! jq -e '
  .status == "healthy" and
  .postgres == "connected" and
  .redis == "connected"
' "$smoke_dir/health.json" >/dev/null; then
  echo "::error::Mercaria MCP health response does not report healthy dependencies."
  exit 1
fi
...
if ! fetch_json \
  ...
  echo "::error::The external Oxy authorization server is unavailable; rolling Mercaria back cannot repair it."
...
if ! jq -e \
  ...
  echo "::error::The external Oxy authorization-server metadata is invalid; rolling Mercaria back cannot repair it."
  exit "$EXTERNAL_DEPENDENCY_EXIT"
fi
...
if ! jq -e \
  ...
  echo "::error::Mercaria protected-resource metadata does not match its canonical read-only MCP catalog."
  exit 1
fi

challenge_status="$(capture_response challenge "$MCP_ORIGIN/mcp")"
if [[ "$challenge_status" != "401" ]]; then
  echo "::error::Mercaria MCP unauthenticated endpoint returned HTTP $challenge_status (expected 401)."
  exit 1
fi
if ! tr -d '\r' <"$smoke_dir/challenge.headers" \
  | grep -i '^www-authenticate: *Bearer ' \
  | grep -Fqi "resource_metadata=\"$RESOURCE_METADATA_URL\""; then
  echo "::error::Mercaria MCP 401 is missing its exact protected-resource metadata challenge."
  exit 1
fi
...
invalid_status="$(capture_response \
  ...
  --data '{"jsonrpc":"2.0","id":"smoke","method":"initialize","params":{"protocolVersion":"2025-06-18","capabilities":{},"clientInfo":{"name":"mercaria-production-smoke","version":"1.0.0"}}}' \
  "$MCP_ORIGIN/mcp")"
if [[ "$invalid_status" != "401" ]]; then
  echo "::error::Mercaria MCP accepted or mishandled an inactive central token with HTTP $invalid_status (expected 401)."
  exit 1
fi
if ! tr -d '\r' <"$smoke_dir/invalid-token.headers" \
  | grep -i '^www-authenticate: *Bearer ' \
  | grep -Fqi 'error="invalid_token"'; then
  echo "::error::Mercaria MCP inactive-token response is missing the OAuth invalid_token challenge."
  exit 1
fi

cross_host_status="$(capture_response cross-host "$MERCARIA_API_ORIGIN/mcp")"
if [[ "$cross_host_status" != "421" ]]; then
  echo "::error::Mercaria MCP resource-host isolation returned HTTP $cross_host_status (expected 421)."
  exit 1
fi

echo "Mercaria MCP and central OAuth post-deploy smoke checks passed."
